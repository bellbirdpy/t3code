...
export type RecoverThreadTurnInput = CommandInput<"thread.turn.recover">;
...
export const recoverThreadTurn: (input: RecoverThreadTurnInput) => CommandEffect = Effect.fn(
  "EnvironmentCommands.recoverThreadTurn",
)(function* (input) {
  const metadata = yield* timestampedCommandMetadata(input);
  return yield* dispatch({
    ...input,
